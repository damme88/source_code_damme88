...
void CCad_ShowView::OnMouseMove(UINT nFlags, CPoint point) 
{
  ...
  if (middle_down_) {
    x_position_ -= (float)(-point.x + middle_down_pos_.x)/20; 
    y_position_ += (float)(-point.y + middle_down_pos_.y)/20;
...
void CCad_ShowView::BuildAxisesList()
{
  ...
  PrintAxisLabel("X");
	glRasterPos3f(0.0, 1.5, 0.0); // tai vi tri 1.5 tung do
  PrintAxisLabel("Y");	
	glRasterPos3f(0.0, 0.0, 1.5);
  PrintAxisLabel("Z");

	// Tạo mũi tên cho trục x 
	glPushMatrix();
	glColor3ub(255, 0, 0);
	...
	//Tạo mũi tên cho truc z
	glPushMatrix();
	glColor3ub(0, 0, 255); // màu blue
	glTranslatef(0.0, 0.0, 1); // tại vị trí 
	glutSolidCone(0.1, 0.2, 100, 100);	 // hàm vẽ 
	glPopMatrix();
 glEndList();
}
