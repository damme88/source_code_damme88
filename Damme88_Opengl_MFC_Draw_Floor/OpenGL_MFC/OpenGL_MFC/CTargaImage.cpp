...
// Xu ly viec doc file dinh dang .tga va la file image
bool CTargaImage::Load(const char *filename) {
  ...
  // bpp la so bit tren 1 pixel
  // color mode = Bpp/8 suy ra la co bao nhieu byte tren 1 pixel
  ...
  // size of image = Dien tich tinh theo pixel * voi so byte tren 1 bitxel
	m_imageSize = m_width * m_height * colorMode;

	// allocate memory for TGA image data- va thuc hien cap phat so vung nho tuong ung 
